...
void init()                         /* initialize the simulator */
{
  ...
  float jimsrand();
   ...
   // scanf("%d",&nsimmax); 
   printf("packet loss probability [enter 0.0 for no loss]: %f\n",lossprob);
   // scanf("%f",&lossprob);
   
   printf("packet corruption probability [0.0 for no corruption]: %f\n",corruptprob);
   // scanf("%f",&corruptprob);
   
   printf("average time between messages from sender's layer5 [ > 0.0]: %f\n",lambda);
   // scanf("%f",&lambda);
  ...
  struct msg message;
{
  ...
  printf("A is pushing a packet to layer 3, payload is %.20s\n",p.payload);
  tolayer3(0,p);
}
  ...
  struct pkt p;
{
    struct msg message;
  for(int i=0;i<20;i++) {
    message.data[i]=p.payload[i];
  }
  printf("B received a packet, and passing the contained message to layer 5\n");
  tolayer5(1,message);
}
